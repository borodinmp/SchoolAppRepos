    ...
    <div class="form-group row">
        ...
        <div class="col-sm-6">
            <input type="text" name="username" value="<#if user??>${user.username}</#if>"
                   class="form-control ${(usernameError??)?string('is-invalid', '')}" placeholder="Login"/>
            <#if usernameError??>
    ...
    <div class="form-group row">
        ...
        <div class="col-sm-6">
            <input type="text" name="fullName" value="<#if user??>${user.fullName}</#if>"
                   class="form-control ${(fullNameError??)?string('is-invalid', '')}" placeholder="Full name"/>
            <#if fullNameError??>
                      <div class="invalid-feedback">
                          ${fullNameError}
                      </div>
...
<form action="/logout" method="post">
    <input type="hidden" name="_csrf" value="${_csrf.token}"/>
    <button class="btn btn-primary" type="submit"><#if Session.SPRING_SECURITY_CONTEXT??>Выход<#else>
            Вход</#if></button>
</form>
</#macro>